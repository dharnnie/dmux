...
import { parsePlanArtifact, parseReviewArtifact } from '../src/handoffs.js';
...
describe('parseReviewArtifact', () => {
  it('parses a well-formed review with verdict + findings', () => {
    const md = `# Review
...
\`\`\`json
{
  ...
  "summary": "Two findings worth addressing.",
  "findings": [
    { "severity": "medium", "file": "src/x.js", "line": 42, "comment": "bcrypt rounds too low" },
    { "severity": "low", "file": "src/x.js", "comment": "missing rate limit" }
  ]
}
\`\`\`
`;
    const r = parseReviewArtifact(md);
    expect(r.parseError).toBeUndefined();
    expect(r.structured.verdict).toBe('request_changes');
    expect(r.structured.summary).toBe('Two findings worth addressing.');
    expect(r.structured.findings).toHaveLength(2);
    expect(r.structured.findings[0]).toEqual({
      severity: 'medium', file: 'src/x.js', line: 42, comment: 'bcrypt rounds too low',
    });
    expect(r.structured.findings[1].line).toBeNull();  // line absent → null
  });

  it('accepts a clean approve with no findings', () => {
    const md = `# Review
\`\`\`json
...
`;
    const r = parseReviewArtifact(md);
    expect(r.structured.verdict).toBe('approve');
    expect(r.structured.findings).toEqual([]);
  });

  it('normalizes unknown verdicts to approve (rather than failing)', () => {
    const md = `# Review
\`\`\`json
{ "verdict": "lgtm", "summary": "looks good", "findings": [] }
\`\`\`
`;
    const r = parseReviewArtifact(md);
    expect(r.structured.verdict).toBe('approve');
  ...
  it('normalizes unknown severities to medium', () => {
    const md = `# Review
\`\`\`json
{
  "verdict": "request_changes",
  "findings": [{ "severity": "spicy", "file": "x.js", "comment": "hot" }]
}
\`\`\`
`;
    const r = parseReviewArtifact(md);
    expect(r.structured.findings[0].severity).toBe('medium');
  });

  it('drops findings missing file or comment', () => {
    const md = `# Review
\`\`\`json
{
  "verdict": "block",
  "findings": [
    { "severity": "high", "file": "ok.js", "comment": "kept" },
    { "severity": "high", "comment": "no file" },
    { "severity": "high", "file": "no-comment.js" },
    { "severity": "high", "file": "", "comment": "empty file" }
  ]
}
\`\`\`
`;
    const r = parseReviewArtifact(md);
    expect(r.structured.findings).toHaveLength(1);
    expect(r.structured.findings[0].file).toBe('ok.js');
  });

  it('returns parseError when no json block present', () => {
    const md = `# Review: no appendix\n\nProse only.`;
    const r = parseReviewArtifact(md);
    expect(r.structured).toBeNull();
    expect(r.parseError).toMatch(/no fenced/);
  });

  it('returns parseError when json is empty object', () => {
    const md = `# Review\n\`\`\`json\n{}\n\`\`\``;
    const r = parseReviewArtifact(md);
    expect(r.structured).toBeNull();
  });

  it('drops non-positive line numbers', () => {
    const md = `# Review
\`\`\`json
{
  "verdict": "approve",
  "findings": [
    { "severity": "low", "file": "x.js", "line": 0, "comment": "zero" },
    { "severity": "low", "file": "x.js", "line": -5, "comment": "negative" },
    { "severity": "low", "file": "x.js", "line": "42", "comment": "string" }
  ]
}
\`\`\`
`;
    const r = parseReviewArtifact(md);
    expect(r.structured.findings.every((f) => f.line === null)).toBe(true);
  });
});
